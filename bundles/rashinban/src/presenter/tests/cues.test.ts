...
test('submission identity is stable across repeated state versions and never inferred from timeout', () => {
  ...
  assert.deepEqual(submitted.cues.filter(c => c.kind === 'guess'), [{ id: `${next.gameId}/${next.round}/${next.players[1].id}/guess`, kind: 'guess', sound: 'opponent-guess', playerId: next.players[1].id, atMs: 1200, untilMs: 1450 }]);
